  ...
  methods: {
          ...
          for (var i = 0; i < n.length; i++) {
            if (
              ...
              c =
                ...
                (n[i] == "power" ? "л.с." : "") +
                (n.length > 1 && this.vi(this.list[n[i]]) != "" ? "/" : "");
            }
          }
        } else {
    ...
    point: function (t) {
      if (this.list.block[t].type != 0) {
        ...
        this.show_galery(
          [{ img: this.list.block[t].img, comment: this.list.block[t].comment }],
          1
        );
      }
    },
    ...
    /**
     * Проверяет обект на существование, если обьект есть вернет
     * @val string object list name
     *
     * @return int length object
     */
    check_len_obj: function (i) {
